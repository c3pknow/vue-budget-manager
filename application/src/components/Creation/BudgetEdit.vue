<template>
  <div class="l-budget-creation">
    <v-layout row wrap>
      ...
      <v-flex xs12 md2 offset-md10>
        <v-btn block color="md-add-item-btn green lighten-1"
                    @click.native="fixClientNameAndUpdate(budget)">Update</v-btn>
</v-flex>
    </v-layout>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    parseBudget() {
      ...
      for (const key in this.selectedBudget) {
        ...
        if (key === "items") {
          ...
          const parseItems = i => i.map(buildItems);
