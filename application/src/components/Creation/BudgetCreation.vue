<template>
  <div class="l-budget-creation">
    ...
    <v-layout row wrap v-for="item in budget.items"
                        class="l-budget-item"
                        :key="item.id">
      <v-flex xs12 md1>
        <v-bth block dark color="red lighten-1" @click.native="removeItem(item)">Remove</v-bth>
      </v-flex>
      ...
      <v-flex xs12 md2 offset-md10>
        <v-btn block color="md-add-item-btn light-blue lighten-1"
                      @click.native="addItem()">Add item</v-btn>
      </v-flex>

      <v-flex xs12 md2 offset-md10>
        <span class="md-budget-item-subtotal white--text">TOTAL $ {{ budget.total_price }}</span>
      </v-flex>

      <v-flex xs12 md2 offset-md10>
        <v-btn block color="md-add-item-btn light-blue lighten-1"
                      @click.native="saveBudget(budget)">Save</v-btn>
      </v-flex>
